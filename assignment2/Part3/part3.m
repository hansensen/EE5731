...
lambda = get_lambda(connected_pixels, epsilon, Ws, im1);
PAIRWISE = sparse(connected_pixels(:,1),connected_pixels(:,2), ...
double(lambda),num_pixels,num_pixels,4*num_pixels);
...
%%
depth_map = mat2gray(reshape(disparity(DispClass+1), W, H)');
figure
imshow(depth_map);
imwrite(depth_map, 'img_disp_0.001:0.001:0.01_18_20.jpg');
toc